...
## 26. Parity-first HDC-Lite

The first HDC architecture uses the fact that the encoder already has the completed ParityGrid.

It searches for a hole mask that is easy for the fixed decoder to reconstruct, rather than asking the decoder to discover an arbitrary puzzle structure.

~~~text
ParityGrid
 -> 36 fixed 9+9+7 masks
 -> exact HDE replay
 -> exact-Parity filter
